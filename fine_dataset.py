...
print(len(passage_index_en))
print(len(passage_index_ko))
quit()
...
class MarcoDataset(Dataset):
    def __init__(self, tokenizer, data_args: DataArguments, trainer = None):
        ...
        self.queries_en_df = queries_en_df
        self.queries_ko_df = queries_ko_df
        self.qrels_df = qrels_df
        self.real_len = len(queries_en_df)
        self.passage_len = len(passage_index_en)

        self.trainer = trainer
    
    def __len__(self):
        return self.real_len * 8 # we will use en-en, en-ko, ko-en, ko-ko combinations and other 4 is for negative samples
    ...
    def __getitem__(self, idx):
        ...
        if len(negative_pids) < negative_size:
            negs = random.choices(negative_pids, k=negative_size)
        ...
        if negative :
            while True :
                idx = random.randint(0, self.passage_len - 1)
                passage_json = get_passage_at('en' if passage_lang_en else 'ko', idx)
                passage_text = passage_json['passage']
                if passage_json['pid'] not in relevant_pids :
                    break
        else :
            for pid in relevant_pids :
                ...
                break # Currently we need only one to simplify
